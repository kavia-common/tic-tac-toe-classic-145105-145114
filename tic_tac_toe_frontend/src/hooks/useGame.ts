...
export function useGame() {
  ...
  const reset = useCallback(
    (playerStarts?: boolean) => {
      const start =
        typeof playerStarts === 'boolean' ? initialPlayer(playerStarts) : initialPlayer(settings.playerStarts);
      setBoard(emptyBoard());
      setTurn(start);
  ...
  const move = useCallback(
    ...
    [addDraw, addWin, board, end.status, turn]
  );
  ...
  const aiShouldMove = useMemo(() => {
    ...
    return (playerIsX && turn === 'O') || (!playerIsX && turn === 'X');
  }, [settings.playerStarts, settings.vsAI, turn]);
  ...
  }, [aiShouldMove, board, end.status, move, settings.aiLevel, turn]);
